...
- Habit targets (5 days of after-meal movement, 5 active days, 5 nights of 7 to 9 hours' sleep) and the 90-minute after-meal window are app-chosen motivation targets, not clinical thresholds. After-meal movement is inferred from timestamps: an activity logged within 90 minutes after a meal counts, so late logging or back-dated entries can miss or over-count it.
- The personal after-meal pattern is correlational and based on few, self-logged meals. Its settings (4-week window, 60 to 150 minute reading window, 5 meals per group, 0.5 mmol/L before calling a difference) are app-chosen. Sensor readings flagged for interstitial lag after meals are excluded as suspect, so in practice the pattern relies mostly on finger-prick checks. Showing a glucose difference can be read as cause; the wording says otherwise. The owner uses it personally without clinician review; it would need clinician approval before the app is shared with anyone else.
...
The app is currently for its owner's personal use. Do not distribute it to other people for unsupervised clinical use until all unchecked items in [WORK_ITEMS.md](WORK_ITEMS.md) are resolved and the [physical-device test plan](PHYSICAL_DEVICE_TEST_PLAN.md) has signed evidence for the supported matrix.
